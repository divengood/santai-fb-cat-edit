 ...
 * @param uploadPreset The name of your unsigned upload preset on Cloudinary.
 * @param logger Optional logger instance.
 * @returns A promise that resolves with the secure public URL for the video.
 */
export const uploadVideo = async (file: File, cloudName: string, uploadPreset: string, logger?: Logger): Promise<string> => {
  if (!cloudName || !uploadPreset) {
    throw new Error("Cloudinary Cloud Name and Upload Preset are required.");
  }
  ...
  logger?.info(`Uploading video "${file.name}" to Cloudinary cloud "${cloudName}"...`);
  const formData = new FormData();
  formData.append('file', file);
  formData.append('upload_preset', uploadPreset);
  ...
  try {
    ...
    logger?.error(`Failed to upload video "${file.name}"`, error);
    throw error;
  }
};
